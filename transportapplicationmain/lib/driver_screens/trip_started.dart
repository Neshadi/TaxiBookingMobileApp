...
class TripStarted extends StatefulWidget {
  ...
  const TripStarted({super.key, required this.userRideRequestDetails});
  ...
  void _onNearDestination() {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text("You are near the destination"),
          content: const Text("You are within 100 meters of the destination"),
          actions: [
            TextButton(
              onPressed: () {
        ...
        drawer: Drawer(
          child: ListView(
            ...
            children: <Widget>[
              ...
              ListTile(
                title: Text(
                    'User Name: ${widget.userRideRequestDetails?.userName ?? "N/A"}'),
              ),
              ListTile(
                title: Text(
                    'User Phone: ${widget.userRideRequestDetails?.userPhone ?? "N/A"}'),
              ),
              ListTile(
                title: Text(
                    'Origin Address: ${widget.userRideRequestDetails?.originAddress ?? "N/A"}'),
              ),
              ListTile(
                title: Text(
                    'Destination Address: ${widget.userRideRequestDetails?.destinationAddress ?? "N/A"}'),
              ),
              ListTile(
                title: Text(
                    'Service Type: ${widget.userRideRequestDetails?.serviceType ?? "N/A"}'),
              ),
              ListTile(
                title: Text(
                    'Capacity: ${widget.userRideRequestDetails?.capacity ?? "N/A"}'),
              ),
              ListTile(
                title: Text(
                    'Weight: ${widget.userRideRequestDetails?.weight ?? "N/A"}'),
              ),
              ListTile(
                title: Text(
                    'Instructions: ${widget.userRideRequestDetails?.instructions ?? "N/A"}'),
              ),
            ],
          ),
        ),
